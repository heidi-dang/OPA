...
import type { PrivacyConfig } from './tools/privacy_security.js';
...
import { executeAnonymousWorkflow, quickAnonymousSetup, quickAnonymousRecon, fullAnonymousWorkflow, getAnonymousStatus } from './tools/anonymous_agent.js';
...
import { getSystemHealth, formatHealthReport } from './tools/health_monitor.js';
import { performSecurityAudit, quickSecurityAudit } from './tools/security_audit.js';
...
export const OPA_Plugin = {
    ...
    tools: {
        ...
        execute_strategy: {
            ...
            execute: async (target: string, args: string) => {
        ...
        security_audit: {
            description: "Performs comprehensive security audit with scoring and detailed recommendations across all code categories.",
            execute: performSecurityAudit,
            requiresApproval: false // Security audit and analysis
        },
        quick_security_audit: {
            description: "Performs quick security audit with scoring summary and key metrics.",
            execute: quickSecurityAudit,
            requiresApproval: false // Quick security assessment
        },
        automation_history: {
        ...
        full_anonymous_workflow: {
            ...
            requiresApproval: false // Complete anonymous operations
        },
        windsurf_workflow: {
            description: "Executes complete WindSurf workflow with automatic privacy setup, reconnaissance, security analysis, and intelligence gathering.",
            execute: async (target: string, args: string) => {
                const workflow = args.includes('recon') ? 'recon' : 
                                 args.includes('security') ? 'security' : 
                                 args.includes('intelligence') ? 'intelligence' : 
        ...
        system_health: {
            description: "Gets a detailed overview of the OPA system health, including sandbox, privacy, and agent status.",
            execute: getSystemHealth,
            requiresApproval: false
        },
        health_report: {
            description: "Generates a comprehensive human-readable report of the system's health status.",
            execute: formatHealthReport,
            requiresApproval: false
        }
    }
};
