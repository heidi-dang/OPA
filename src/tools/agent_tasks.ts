...
export class AgentTaskManager {
    private config: AgentTaskConfig;
     ...
     */
    async executeTask(taskName: string, taskFunction: () => Promise<string>, target?: string): Promise<string> {
     ...
     */
    private isDeploymentError(error: any): boolean {
     ...
     */
    private async executeWithE2BFallback(taskName: string, taskFunction: () => Promise<string>): Promise<string> {
        try {
            // Force E2B sandbox
     ...
     */
    private addExecutionMetadata(result: string, taskName: string, executionTime: number, sandboxType: string = 'Local'): string {
        const metadata = `
╔══════════════════════════════════════════════════════════════╗
...
`;
     ...
     */
    async executeBatchTasks(tasks: Array<{
        name: string;
 ...
 */
export function getAgentTaskManager(config?: AgentTaskConfig): AgentTaskManager {
    if (!agentTaskManager) {
 ...
 * Execute a task with automatic sandbox deployment
 */
export async function executeAgentTask(taskName: string, taskFunction: () => Promise<string>, target?: string): Promise<string> {
 ...
 */
export async function executeBatchAgentTasks(tasks: Array<{
    name: string;
    function: () => Promise<string>;
    priority?: 'high' | 'medium' | 'low';
}>): Promise<string> {
 ...
 */
export function resetAgentDeploymentCheck(): void {
    const agent = getAgentTaskManager();
