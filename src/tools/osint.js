...
export async function osintRecon(target, options = '') {
...
function parseDNSRecords(dnsOutput) {
    const records = {
        A: [],
        AAAA: [],
        MX: [],
...
function extractTechnologies(xmlOutput) {
    const technologies = [];
...
function parseWhois(whoisOutput) {
    ...
    lines.forEach(line => {
        if (line.includes('Registrar:')) {
            ...
            if (registrantEmail)
                info.registrantEmail = registrantEmail;
        }
    });
    return info;
}
function extractSocialAccounts(socialOutput, target) {
    const accounts = [];
...
function generateOSINTReport(result) {
    let report = `
╔══════════════════════════════════════════════════════════════╗
