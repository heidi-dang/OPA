...
async function executeTaskBatch(tasks, priority) {
...
export async function parallelScan(target) {
    const tasks = [
        { name: 'nmap_scan', target, priority: 'high', args: '-sS -sV -O' },
        { name: 'osint_recon', target, priority: 'medium' },
        { name: 'run_nuclei', target, priority: 'medium', args: '-t cves' },
        { name: 'dir_fuzz', target, priority: 'low' }
    ];
