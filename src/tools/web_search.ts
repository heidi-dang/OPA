...
export class WebSearchAgent {
     ...
     */
    private async searchWithEngine(engine: string): Promise<SearchResult[]> {
        ...
        const command = searchCommands[engine as keyof typeof searchCommands];
     ...
     */
    private async searchCVEs(): Promise<SearchResult[]> {
     ...
     */
    private async searchCompanyData(): Promise<SearchResult[]> {
     ...
     */
    private async searchNews(): Promise<SearchResult[]> {
     ...
     * Search social media
     */
    private async searchSocial(): Promise<SearchResult[]> {
     ...
     */
    private generateSearchReport(results: SearchResult[]): string {
        return `
╔══════════════════════════════════════════════════════╗
║                    WEB SEARCH RESULTS                          ║
╚═════════════════════════════════════════════════════
...
let webSearchAgent: WebSearchAgent | null = null;

/**
 * Get or create web search agent
 */
export function getWebSearchAgent(): WebSearchAgent {
    return webSearchAgent || (webSearchAgent = new WebSearchAgent({} as WebSearchConfig));
}

/**
 * Perform web search
 */
export async function webSearch(query: string, options: Partial<WebSearchConfig> = {}): Promise<string> {
 ...
 */
export async function securityWebSearch(query: string): Promise<string> {
    return await webSearch(query, {
        maxResults: 10,
        engines: ['google', 'bing'],
        includeCVE: true,
 ...
 */
export async function intelligenceSearch(query: string): Promise<string> {
    return await webSearch(query, {
        maxResults: 15,
